      ...
      class Tesseract {
         ...
         OCR(image, language:="", options:=""){
            ...
            try {
               ...
               this.convert_optimized(this.fileProcessedImage, this.fileConvertedText)
         ...
         convert(in:="", out:="", fast:=1){
            ...
            ; The item properties area is a compact block of one or more lines.
            ; Automatic page detection can return an empty result for this shape.
            _cmd .= " --psm 6"
         ...
         convert_optimized(in:="", out:=""){
            ; On small Vermintide 2 Chinese text, chi_sim fast preserves joined
            ; property names more reliably than the legacy best model.
            return (this.language = "chi_sim")
               ? this.convert_fast(in, out)
               : this.convert_best(in, out)
         }
         ...
         preprocess(in:="", out:=""){
            ...
            ; Vermintide 2 property text is often only 10-15 pixels high.
            ; Preserve anti-aliased strokes and add a border while scaling for
            ; every OCR language. The old 1x Leptonica path could turn 33.0%
            ; into 33.09%, causing the safe range check to reject the line.
            return this.preprocess_property_text(in, out)
         }

         preprocess_property_text(in, out){
            scaleFactor := 4
            border := 20
            pToken := Gdip_Startup()
            pBitmap := 0
            ...
            if (!pToken)
               throw Exception("GDI+ startup failed during property OCR preprocessing.")

            try {
               pBitmap := Gdip_CreateBitmapFromFile(in)
               if (!pBitmap)
                  throw Exception("Property OCR source image could not be opened.",, in)

               width := Gdip_GetImageWidth(pBitmap)
               height := Gdip_GetImageHeight(pBitmap)
               outputWidth := width * scaleFactor + border * 2
               outputHeight := height * scaleFactor + border * 2
               pScaledBitmap := Gdip_CreateBitmap(outputWidth, outputHeight)
               pGraphics := Gdip_GraphicsFromImage(pScaledBitmap)
               Gdip_GraphicsClear(pGraphics, 0xFF000000)
               Gdip_SetInterpolationMode(pGraphics, 7)
               DllCall("gdiplus\GdipSetPixelOffsetMode", "Ptr", pGraphics, "Int", 4)
               Gdip_DrawImage(pGraphics, pBitmap, border, border
                  , width * scaleFactor, height * scaleFactor, 0, 0, width, height)

               if (Gdip_SaveBitmapToFile(pScaledBitmap, out) != 0)
                  throw Exception("Property OCR processed image could not be saved.",, out)
            } finally {
               if (pGraphics)
                  Gdip_DeleteGraphics(pGraphics)
               if (pScaledBitmap)
            ...
            if !(FileExist(out))
               throw Exception("Property OCR preprocessing failed.",, out)
